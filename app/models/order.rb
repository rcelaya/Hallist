...
class Order < ActiveRecord::Base
  ...
  state_machine :initial => 'in_progress' do
    ...
    after_transition to: 'complete', do: [:set_counters]
  ...
  def find_total(force = false)
    ...
    self.find_sub_total
    self.total = (self.total + shipping_charges - deal_amount - coupon_amount ).round(2)
  end
  ...
  def credited_total
    #ARTURO!!!!#problem with round_at method
  ...
  def shipping_charges(items = nil)
    ...
    @order_shipping_charges = shipping_rates(items).inject(0.0) {|sum, shipping_rate|  sum + (shipping_rate.try(:rate) || 0)  }
  end
  ...
  def shipping_rates(items = nil)
    ...
    rates = items.inject([]) do |rates, item|
      rates << item.shipping_rate if item.shipping_rate.try(:individual?) || !rates.include?(item.shipping_rate)
      rates
    end
  end
  ...
  def default_shipping_rate
    order_items.each do |order_item|
      order_item.update_attribute(:shipping_rate_id,ShippingRate.first.id)
    end
  end
  ...
  def create_2checkout_invoice(response, charge_amount)
    transaction do
      invoice_statement = Invoice.generate(self.id, charge_amount, 0.0)
      invoice_statement.save
      invoice_statement.twocheckout_payment(response)
      invoices.push(invoice_statement)
      order_complete!
      save!
    end
  ...
  def update_tax_rates
    if ship_address_id_changed?
      ...
      tax_time = completed_at? ? completed_at : Time.zone.now
      order_items.each do |item|
        rate = item.variant.product.tax_rate(self.ship_address.state, tax_time)
  ...
  def set_counters
    self.user.shoppings_counter += 1
    self.user.save
    self.order_items.each do |order_item|
      artist = order_item.variant.product.brand.artist
      artist.sells_counter +=1
      artist.save!
    end
  end
end
