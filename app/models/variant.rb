...
class Variant < ActiveRecord::Base
  ...
  def size=(size_hash)
    variant_property = variant_properties.find_or_initialize_by_property_id(property_size.id) 
    variant_property.description = "#{size_hash[:width]} x #{size_hash[:height]} #{size_hash[:measures]}"
  end
  
  def size
    variant_property = variant_properties.find_or_initialize_by_property_id(property_size.id) 
    result = variant_property.description.match /^(\d+) x (\d+) (.*)$/
    if result
      {'width' => result[1], 'height' => result[2], 'measures' => result[3]}
    else
      result = variant_property.description.match /^(\d+) x (\d+)$/
      {'width' => result[1], 'height' => result[2], 'measures' => nil}
    end
  end
  ...
  def property_size
    @property_size ||= (properties.where(identifing_name: 'Size').first || Property.find_by_identifing_name('Size'))
  end
