class ApplicationController < ActionController::Base
  ...
  def set_search
    if request.path == "/search"
      redirect_to search_term_url(search_term: params[:filters][:search])
    elsif request.path == "/collections/search"
      redirect_to collections_search_term_url(search_term: params[:filters][:search])
    end
  end
  ...
  def session_cart!
    ...
    elsif current_user.user && current_user.current_cart
  ...
  def most_likely_user
    current_user.user ? current_user : random_user
  end
  ...
  def current_user
    return UserDecorator.decorate(@current_user) if defined?(@current_user)
