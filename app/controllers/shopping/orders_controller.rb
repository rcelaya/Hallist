class Shopping::OrdersController < Shopping::BaseController
  ...
  def index
    ...
    if f = next_form(@order)
      ...
      form_info
      redirect_to shopping_checkout_url()
    ...
    if @order.complete?
      ...
      if response = @order.create_invoice(@credit_card,
        ...
        if response.succeeded?
          ...
          redirect_to shopping_checkout_url, alert: [I18n.t('could_not_process'), I18n.t('the_order')].join(' ')
        end
      else
        redirect_to shopping_checkout_url, alert: [I18n.t('could_not_process'), I18n.t('the_credit_card')].join(' ')
      end
    else
      redirect_to shopping_checkout_url, alert: [I18n.t('credit_card'), I18n.t('is_not_valid')].join(' ')
    end
  end
